...
class CacheExchangeCalculatorDecorator implements ExchangeCalculator
{
    ...
    public function getExchangeRatioForCurrencies(
        ...
        if (!isset($this->exchangesCache[$firstCurrency->getValue()][$secondCurrency->getValue()])) {
            $this->exchangesCache[$firstCurrency->getValue()][$secondCurrency->getValue(
            )] = $this->exchangeCalculatorChild->getExchangeRatioForCurrencies(
                $firstCurrency->getValue(),
                $secondCurrency->getValue()
            );
        }
